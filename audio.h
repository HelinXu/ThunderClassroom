...
【接口说明】
    ...
    函数StartPlay开始播放
    函数StopPlay停止播放
    函数GetAndRemoveFirstRecorded获取并删除最早录制的一段音频
    函数AppendFrameToPlay向待播放队列最后追加一段音频
    函数Clear清除全部已录制的音频和待播放的音频
    函数RecordFramesCount已录制音频队列里的音频数量(单位为秒，整数)
    函数ToPlayFramesCount待播放音频队列里的音频数量(单位为秒，整数)

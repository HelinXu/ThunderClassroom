...
【更改记录】
    ...
    2020-06-18 徐赫临添加了是否正在尝试链接的判断
*************************************************************************/
StudentProcess::StudentProcess(QObject* Parent): QObject(Parent), IsInClass(m_bIsInClass), IsTryingToConnect(m_bIsTryingToConnect)
{
...
bool StudentProcess::EnterClass(const QString& IP) {
    ...
    //30秒内未连接，则继续循环
    }while(EndTime - BeginTime <= 10000 && !m_pClient->IsConnected);
...
bool StudentProcess::ExitClass() {
    ...
    //m_pClient->Send(Message(MSG_ATTENTION,))
    //断开与服务器连接
...
【返回值】true表示当前连接状态（是否在课堂里）
【开发者及日期】范静涛(fanjingtao@tsinghua.edu.cn) 2020-5-12
【更改记录】徐赫临 2020-06-18 更改为可选择群发或者只发给老师。
*************************************************************************/
bool StudentProcess::Send(const QString& Msg, bool IsToAll) {
    //不在课堂
    if (!m_bIsInClass) {
        return m_bIsInClass;
    }
    ...
    //如果是发给全体的
        ...
        //装换格式并发送
        qDebug() << "SP: send Message to all: " << Msg;
        string MsgStr = Msg.toStdString();
        return m_pClient->Send(Message::FromStringToAll(MsgStr));
    }
}

/*************************************************************************
【函数名称】StudentProcess::HandInMtpChoice(unsigned int ProblemNumber,
                                     unsigned int Time,
                                     bool C1, bool C2,
                                     bool C3, bool C4)
【函数功能】向教师端发送本题作答
...
【更改记录】徐赫临 2020-06-20 徐赫临做了代码规范修改
*************************************************************************/
bool StudentProcess::HandInMtpChoice(unsigned int ProblemNumber,
                                     unsigned int Time,
                                     bool C1, bool C2,
                                     bool C3, bool C4) {
    //不在课堂
    ...
    //@TODO
    return m_pClient->Send(Message::FromAnswerMtpChoice(ProblemNumber, Time, C1, C2, C3, C4));
}

/*************************************************************************
【函数名称】StudentProcess::SendAttention(double Attention)
【函数功能】向教师端发送注意力
【参数】入口参数，答题的各种信息
【返回值】true表示当前连接状态（是否在课堂里）
【开发者及日期】徐赫临 2020-06-14
【更改记录】徐赫临 2020-06-20 徐赫临做了代码规范修改
*************************************************************************/
bool StudentProcess::SendAttention(double Attention) {
    //不在课堂
    if (!m_bIsInClass) {
        return m_bIsInClass;
    }
    ...
    //@TODO
    return m_pClient->Send(Message::FromAttention(Attention));
}
...
【更改记录】
    ...
    2020-06-18 徐赫临增加了其他收发消息类型的处理结果
*************************************************************************/
DWORD WINAPI StudentProcess::MsgReceiveThread(LPVOID lpParameter) {
    ...
    while (pProcess->IsInClass && pProcess->m_pClient->IsConnected) {
        ...
        if (pProcess->m_pClient->GetAndRemoveFirstRecvMsg(TempMessage)) {
            ...
            else if (Type == MSG_CLASSTOEND) {
                emit pProcess->ClassToEnd();
            }
            else if (Type == MSG_MTPCHOICE) {
                bool IsSingleChoice;
                unsigned int ProblemNum;
                unsigned int TimeLimit;
                string Text = Message::ToMtpString(TempMessage, TimeLimit, ProblemNum, IsSingleChoice);
                emit pProcess->RecvProblem(QString::fromStdString(Text), TimeLimit, ProblemNum, IsSingleChoice);
            }
            //如果是音频帧消息，放入待播放音频队列中
            ...
            else if (Type == MSG_STOPANSWERING) {
                emit pProcess->StopAnswering();
            }
            else if (Type == MSG_TOALLSTRING) {
                qDebug() << "SP: recieved send_toAll chat message: " << QString::fromStdString(Message::To_to_all_String(TempMessage));
                emit pProcess->RecvChat(QString::fromStdString(Message::To_to_all_String(TempMessage)));
            }
        }
    }
    return 0;

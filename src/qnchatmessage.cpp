...
/*************************************************************************
【函数名称】QNChatMessage::QNChatMessage(QWidget *parent)
【函数功能】构造函数
【参数】无
【返回值】构造函数不可有返回值
【开发者及日期】# Author: 沙振宇
...
QNChatMessage::QNChatMessage(QWidget *parent) : QWidget(parent)
{
    ...
    m_leftPixmap = QPixmap(":/ChatWindow/image/Cat.png");
    m_rightPixmap = QPixmap(":/ChatWindow/image/fox.png");
...
/*************************************************************************
【函数名称】QNChatMessage::setTextSuccess()
【函数功能】设置聊天序列
【参数】无
【返回值】无
【开发者及日期】# Author: 沙振宇
...
/*************************************************************************
【函数名称】QNChatMessage::setText(QString text, QString time, QSize allSize, QNChatMessage::User_Type userType)
【函数功能】设置聊天文字
【参数】无
【返回值】无
【开发者及日期】# Author: 沙振宇
...
QSize QNChatMessage::fontRect(QString str)
{
    ...
    m_sanjiaoLeftRect = QRect(iconWH + iconSpaceW + iconRectW, m_lineHeight / 2,
                              sanJiaoW, hei - m_lineHeight);
    m_sanjiaoRightRect = QRect(this->width() - iconRectW - iconWH - iconSpaceW - sanJiaoW,
                               m_lineHeight / 2, sanJiaoW, hei - m_lineHeight);

    if(size.width() < (m_textWidth+m_spaceWid)) {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x() + m_sanjiaoLeftRect.width(),
                                m_lineHeight / 4 * 3, size.width() - m_spaceWid
                                + 2 * textSpaceRect, hei - m_lineHeight);
        m_kuangRightRect.setRect(this->width() - size.width() + m_spaceWid
                                 - 2 * textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW,
                                 m_lineHeight / 4 * 3, size.width() - m_spaceWid
                                 + 2 * textSpaceRect, hei - m_lineHeight);
    } else {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(),
                                m_lineHeight / 4 * 3, m_kuangWidth, hei - m_lineHeight);
        m_kuangRightRect.setRect(iconWH + kuangTMP + iconSpaceW + iconRectW - sanJiaoW,
                                 m_lineHeight / 4 * 3, m_kuangWidth, hei - m_lineHeight);
    }
...
/*************************************************************************
【函数名称】QNChatMessage::getRealString(QString src)
【函数功能】设置聊天文字
【参数】无
【返回值】无
【开发者及日期】# Author: 沙振宇
...
/*************************************************************************
【函数名称】QNChatMessage::paintEvent(QPaintEvent *event)
【函数功能】绘制图形界面
【参数】无
【返回值】无
【开发者及日期】# Author: 沙振宇
...
void QNChatMessage::paintEvent(QPaintEvent *event)
{
    ...
    if(m_userType == User_Type::User_She) { // 用户
        ...
        painter.drawLine(QPointF(m_sanjiaoLeftRect.x() - 1, 30),
                         QPointF(m_sanjiaoLeftRect.x() + m_sanjiaoLeftRect.width(), 24));
        painter.drawLine(QPointF(m_sanjiaoLeftRect.x() - 1, 30),
                         QPointF(m_sanjiaoLeftRect.x() + m_sanjiaoLeftRect.width(), 36));

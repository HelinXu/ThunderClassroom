...
public:
    ...
    //学生群发给所有人的消息
    static Message FromStringToAll(const string& aString);
    ...
    //静态成员函数，构造特定类型的消息
    static Message FromMtpChoice(const MultipleChoice& aMtpChoice);

    //静态成员函数，构造特定类型的消息
    static Message FromAnswerMtpChoice(unsigned int ProblemNumber,
                                       unsigned int Time,
                                       bool C1, bool C2,
                                       bool C3, bool C4);

    //静态成员函数，构造特定类型的消息
    static Message FromAttention(double Attention);

    //静态成员函数，还原消息为特定类型对象
    static string ToString(const Message& Msg);

    //转发给所有人的消息
    static string To_to_all_String(const Message& Msg);
    ...
    //静态成员函数，还原消息为特定类型对象
    static string ToMtpString(const Message& MtpChoice,
                              unsigned int& TimeLimit,
                              unsigned int& MtpChoiceNum,
                              bool& IsSingleChoice);

    //静态成员函数，还原消息为特定类型对象
    static void ToAnswerMtpChoice(const Message& AnswerData,
                                  bool& C1, bool& C2,
                                  bool& C3, bool& C4,
    ...
    //静态成员函数，还原消息为特定类型对象
    static unsigned int ToAttention(const Message& Msg);
